...
at the *deref coersion* feature of Rust and how that lets us work with either
references or smart pointors.
...
The compliation error we get is:
...
type that `Deref` can convert the original type into. Deref coersion happens
...
If Rust didn't implement deref coersion, in order to call `hello` with a value
...
<!-- Would it make sense to move this list to the start of the deref setion? -->
